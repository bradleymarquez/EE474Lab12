 ...
 * 24 / May / 2016
 *
 * Connects to a bluetooth device, and takes in inputs
...
#define BUF_MAX 256 // size of buffer
#define TIMEOUT 10 // read timeout
...
char *RXTX_path = "/dev/ttyO1"; // Bluetooth Path
bool setup = false;
...
int main(){
	...
	configureBluetooth();
	timer_read_Init(); // Reads at a certain rate
	while (1) {
	}
	return 0;
}
